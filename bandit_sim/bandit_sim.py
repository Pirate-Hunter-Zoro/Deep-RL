...
class Bandit_Sim:
    ...
    """
    def __init__(self, n_arms, payout_std=0.1, clip=2, seed=None):
    ...
    def pull_arm(self, n):
        ...
        """
        reward = np.random.normal(self.arm_means[n], self.payout_std)
        while abs(reward - self.arm_means[n]) > self.clip * self.payout_std:
            reward = np.random.normal(self.arm_means[n], self.payout_std)
        return reward
